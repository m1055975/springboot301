...
public class OrderServiceImpl implements OrderService {
	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for (OrderedItemsDto itemDto : itemsDtoList) {
			...
			try {
				...
				DeliveryRequestDto delivery = new DeliveryRequestDto(order.getId(), order.getStatus(),
						order.getCustomerId(), order.getRestaurantId());
				kafkaSender.sendData(delivery);
	...
	private OrderUpdateResponseDto defaultMethod(OrderUpdateDto orderUpdateRequest) {

		OrderUpdateResponseDto orderResponse = new OrderUpdateResponseDto();
//		orderResponse.setCustomerId((long) 0);
